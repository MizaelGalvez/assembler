...
copystring:
  ...
  .sigcar:
    mov bl, byte[eax] ; copiamos la direccion del mensje a EBX
    cmp bl, 0xA   ;comparamos con salto de linea
    je .salto

    mov byte[esi+ecx], bl  ; movemos un caracter al array
    cmp byte[eax], 0x0    ;comparamos el en la direccion a la que apunta a eax con convertimos
    jz .finalizar

  .salto:
    inc eax  ; incrementamoes en uno el acumulador
    inc ecx       ; incrementamos el contador
    jmp .sigcar   ;saltamos al siguiente caracter
